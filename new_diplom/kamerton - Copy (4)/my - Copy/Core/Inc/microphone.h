...
/*
 * Drain any DMA half-buffer(s) completed since the last call and run the
 * filtering / recorder / chord feed work.  Must be called once per main-loop
 * iteration, BEFORE MIC_Process() / Task_Recorder().  The heavy work lives
 * here, NOT in the DMA interrupt, so the bit-banged keypad read is never
 * stalled by the microphone.
 */
