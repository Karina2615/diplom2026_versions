...
/* Recorder WAV-playback buffer removed.  On the ESP-32 DevKit V1 (ESP-WROOM-32,
 * no PSRAM) a 256 KB RAM buffer cannot coexist with the Bluetooth A2DP stack and
 * prevented BT from initialising.  Recording to the SD card still works; playing
 * a recording back through the synth is disabled — only live synth audio is
 * streamed over Bluetooth, which is all that is required. */
    ...
    if (keys[1]) {
        if (!g_sdrec.isRecording()) {
            ...
            if (slot > 0) {
                ...
                timerStart(g_rec_timer);
                g_sdrec.startRec(fname);
            }
        } else {
            // Stop recording
            timerStop(g_rec_timer);
            g_sdrec.stopRec();
            refreshRecList();
        }
    }

    // Key 2 (play selected recording) is disabled: there is no RAM playback
    // buffer on WROOM-32 (see the g_play_buf note above).  Recording and SD
    // storage still work; only RAM-based playback of recordings was dropped.
...
// ─── Mic-sampling ISR gating ──────────────────────────────────────────────────
// The 16 kHz onMicTimer() ISR (analogRead in interrupt context) is only needed
// for Tuner pitch detection.  Keeping it off in every other mode frees the
// CPU/ADC for the Bluetooth A2DP audio task, which otherwise underruns and
// streams noise instead of clean tones.
static void updateMicTimer() {
    if (g_mic_timer == nullptr) return;
    if (g_mode == MODE_TUNER) timerStart(g_mic_timer);
    else                      timerStop(g_mic_timer);
}
...
static void onModeSwitch() {
    ...
    // Gate the ADC ISRs: mic sampling only in Tuner, and make sure the
    // recording ISR is off after leaving Recorder.  Both starve BT audio.
    updateMicTimer();
    timerStop(g_rec_timer);
...
static void handleJoystick() {
    ...
    switch (g_mode) {
        ...
        case MODE_RECORDER:
            ...
            // Joystick-button playback disabled: no RAM playback buffer on WROOM-32.
            break;
...
void setup() {
    ...
    // ADC sampling timers (16 kHz).  Both are created but left DISABLED.
    // Running analogRead() inside a 16 kHz ISR continuously starves the
    // Bluetooth A2DP audio task and corrupts the stream (heard as noise), so
    // each timer is enabled only while it is actually needed:
    //   • g_rec_timer — only while recording  (enabled in handleRecorder)
    //   • g_mic_timer — only in Tuner mode     (enabled by updateMicTimer)
    // Core 3.x timer API: timerBegin(freq_hz) sets the tick rate directly.
    // 1 MHz → 1 µs ticks (same resolution the old prescaler-80 setup gave).
    // timerAlarm(..., autoreload=true, reload=0) configures + arms a periodic
    // alarm; timerStop() then pauses the counter so the ISR can't fire until
    // the matching timerStart() in handleRecorder()/updateMicTimer().
    g_rec_timer = timerBegin(1000000);
    timerAttachInterrupt(g_rec_timer, &onRecTimer);
    timerAlarm(g_rec_timer, 1000000 / REC_SAMPLE_RATE, true, 0);
    timerStop(g_rec_timer);   // paused until handleRecorder starts a recording

    g_mic_timer = timerBegin(1000000);
    timerAttachInterrupt(g_mic_timer, &onMicTimer);
    timerAlarm(g_mic_timer, 1000000 / REC_SAMPLE_RATE, true, 0);
    timerStop(g_mic_timer);   // paused until updateMicTimer() needs Tuner sampling
    updateMicTimer();         // honour the restored boot mode (on if it is Tuner)
