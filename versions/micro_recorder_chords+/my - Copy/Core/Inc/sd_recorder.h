 ...
 * File format: raw 16-bit signed PCM, 9070 Hz, mono (.PCM files)
...
/* Last f_mount error code (FR_OK when mounted, FR_NO_FILESYSTEM when wrong format) */
FRESULT  SDRec_GetMountError(void);

/* Format SD card as FAT32 and re-mount; returns true on success.
 * Use when SDRec_GetMountError()==FR_NO_FILESYSTEM (e.g. exFAT 64GB card). */
