...
#define MAX_SAMPLE_SECONDS  2
// ESP-WROOM-32 has NO PSRAM, so every PCM buffer competes for internal DRAM with
// the Bluetooth A2DP stack.  The 256 KB recorder RAM-playback buffer was removed
// (MAX_REC_SAMPLES is no longer allocated).  The sampler keeps one buffer:
//   2 s * 16000 Hz = 32 000 words = 64 KB  — small enough to fit beside BT.
// MAX_REC_SECONDS still bounds recording length written to the SD card.
