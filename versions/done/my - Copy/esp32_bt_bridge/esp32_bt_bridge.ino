...
#include <string.h>   // memset — для добивання буфера тишею
...
#define DMA_BUF_COUNT 24    // більше = довше «перемелює» дрейф/перешкоди (постав 32, якщо ще рве)
#define DMA_BUF_LEN   256   // 24×256 ≈ 139 мс буфера @ 44.1 кГц (зменш, якщо затримка заважає)
...
int32_t audioCallback(Frame* frame, int32_t frameCount) {
    size_t bytesWanted = (size_t)frameCount * sizeof(Frame);
    size_t bytesRead   = 0;

    // Обмежене очікування (НЕ portMAX_DELAY!): якщо STM32 на мить відстав,
    // не блокуємо BT-стек назавжди — саме це «вічне очікування» і давало заїкання.
    esp_err_t err = i2s_read(I2S_NUM_0, (void*)frame, bytesWanted,
                             &bytesRead, pdMS_TO_TICKS(15));
    if (err != ESP_OK) bytesRead = 0;

    // Якщо даних прийшло менше, ніж просять (буфер підтік через дрейф тактів) —
    // добиваємо рештку тишею. Короткий «нуль» майже не чути, а пропуск
    // A2DP-пакета = різке заїкання.
    if (bytesRead < bytesWanted) {
        memset((uint8_t*)frame + bytesRead, 0, bytesWanted - bytesRead);
    }

    // Завжди віддаємо ПОВНИЙ кадр → потік у Bluetooth безперервний.
    return frameCount;
}
