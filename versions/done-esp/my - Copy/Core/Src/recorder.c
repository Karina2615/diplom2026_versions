 ...
 *
 * Buffer layout (96 KB total, 48 000 samples × 2 B ≈ 5.3 s @ 9 070 Hz):
 *   rec_buf[32000] — 64 KB in CCM-SRAM (CPU-only, not DMA-accessible)
 *   rec_ext[16000] — 32 KB in main SRAM (overflow)
 *   rec_rd / rec_wr helpers transparently span both regions.
 *
 * NOTE: rec_ext was halved (32000 -> 16000) to free 32 KB of main SRAM.
 * The full build was using 126.7/128 KB of RAM, leaving only ~4.3 KB for
 * heap+stack; deep call paths (FatFs, snprintf float, mic autocorrelation)
 * overflowed the stack and corrupted .bss globals — including the keypad/
 * joystick state in main.o — causing phantom keys and self-switching modes.
 * Freeing 32 KB restores ~36 KB headroom (the known-good build had 22.6 KB).
 *
...
#define REC_EXT_SIZE   16000u   /* int16_t count in main SRAM (32 KB) — halved to restore stack headroom */
